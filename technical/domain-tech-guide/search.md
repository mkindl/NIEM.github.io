...
NOTE:  Microsoft Internet Explorer 11 browser must run in "compatibility mode" for SSGT to work correctly:
...
Google search results will contain any Web page within the release.niem.gov/niem/ directory that matches <code>PersonType</code>.
...
Note that Google search is limited.  Google only indexes full words (surrounded by space), not full text.  This means that Google search only matches words (or character strings) bound by spaces (or other delimiters) NOT partial matches.  For example, if you enter:

	site:release.niem.gov/niem/ Pers

Search results will only return pages that contain a <code>pers</code> string bound by spaces.
